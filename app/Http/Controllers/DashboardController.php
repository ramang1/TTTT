...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $showinbox = DB::table('inboxes')->leftjoin('process_inbox', 'process_inbox.inboxes_id', '=', 'inboxes.id')
            ...
            ->selectRaw('contacts.name as contacts_name, users.name as users_name,inboxes.name as name, inboxes.size as size, inboxes.created_at as created_at, process_inbox.inboxes_id as inboxes_id, inboxes.id as inboxes_id, process_inbox.action_type as action,inboxes.type as type,contacts.id as contacts_id, users.id as users_id')
            ->orderBy('inboxes.created_at', 'desc')->groupBy('inboxes.id')->take(10)->get();
        //Lay 10 mail di moi nhat trong ngay
        $showoutbox = DB::table('outboxes')->leftjoin('outbox_process', 'outbox_process.outbox_id', '=', 'outboxes.id')
            ->join('contacts', 'contacts.id', '=', 'outboxes.contact_id')
            ->join('users', 'users.id', '=', 'outboxes.user_id')                 
            ->selectRaw('contacts.name as contacts_name, users.name as users_name,outboxes.name as name, outboxes.size as size, outboxes.created_at as created_at,outbox_process.outbox_id as outboxes_id,outboxes.id as outboxes_id,outbox_process.action_type as action,outboxes.type as type, contacts.id as contacts_id, users.id as users_id')
